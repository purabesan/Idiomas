...
自動翻訳やシーンの再スキャン、JSON同期を行う前にCSVをエクスポートしておくと、
翻訳内容を目視で確認しやすいバックアップとして利用できます。
エクスポート済みのCSVは、同じファイルを再度エクスポート先に指定しない限り、
Idiomasの通常処理では変更されません。

重要な翻訳データは、CSVによる作業用バックアップに加えて、
JSONファイルもGitなどでバージョン管理することを推奨します。
